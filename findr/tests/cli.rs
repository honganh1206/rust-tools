...
fn run(args: &[&str], expected_file: &str) -> Result<()> {
    ...
    // How does cloning here get along with borrow-checking?
...
fn unreadable_dir() -> Result<()> {
    ...
    // ? allows us to propagate from Result<Type, Err> to Type
    ...
    let out = cmd.get_output();
    let stdout = String::from_utf8(out.stdout.clone())?;
    // First read from stdout then do some processing
    // then we use closure to filter out empty files
    // then the data is evaluated to an iterator
    // then we collect() it as a collection (vector in this case)
