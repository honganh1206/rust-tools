// Allow Result to use 0 type argument?
use anyhow::Result;
...
// --------------------------------------------------
// Helper function when reading from physical files
...
// --------------------------------------------------
// Helper fuunction when reading from piped input
fn run_stdin(args: &[&str], input_file: &str, expected_file: &str) -> Result<()> {
    ...
    // while replacing invalid UTF-8 characters with placeholders
    let expected = String::from_utf8_lossy(&buffer);
