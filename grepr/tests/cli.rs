...
fn run(args: &[&str], expected_file: &str) -> Result<()> {
    let window_file = format!("{expected_file}.windows");
    let expected_file = if os_type().unwrap() == "Windows" && Path::new(&window_file).is_file() {
        &window_file
    } else {
        expected_file
    };
    ...
    // cargo_bin return a Result :) Why then?
